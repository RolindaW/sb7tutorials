...
#pragma region Object
	...
	void InitializeObject(int mode)
	{
		...
		{
		case 0:
			InitializeObject_01TightlyPackedMultipleBindingPoints();
			break;
		case 1:
			InitializeObject_02InterleavedMultipleBindingPoints();
			break;
		case 2:
			InitializeObject_03InterleavedSharedBindingPoint();
			break;
		default:
			break;
		}
	...
	void InitializeObject_01TightlyPackedMultipleBindingPoints()
	{
		...
		// Warning!Specified by buffer binding point index, not by vertex attribute index
		// Error! Specifing a divisor by an instanced vertex attribute index that is bound to a buffer binding point makes the vertex attribute data not to be correctly processed
		glVertexAttribDivisor(6, 1);  // Saved into active VAO
	...
	void InitializeObject_02InterleavedMultipleBindingPoints()
	{
		// Create and bind VAO (Vertex Array Object)
		glCreateVertexArrays(1, &vao);
		glBindVertexArray(vao);
		...
		// Setup and enable vertex attributes feeding in VAO
		const unsigned int stride = sizeof(GLfloat) * 8;
	...
	void InitializeObject_03InterleavedSharedBindingPoint()
	{
		// Create and bind VAO (Vertex Array Object)
		glCreateVertexArrays(1, &vao);
		glBindVertexArray(vao);
		...
		// Define shared buffer binding point for instanced vertex attributes
		// Note: In this case is possible to use a shared binding point for instanced vertex attributes because they use same divisor value
		glVertexArrayVertexBuffer(vao, 7, vbo, 0, stride);
		glVertexAttribDivisor(7, 1);
	}
