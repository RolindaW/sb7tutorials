...
private:
	...
	GLuint compile_shaders(void)
	{
		...
		static const GLchar* vertex_shader_source[] =
		{
			...
			"	vec3 mixed_color = mix(color, material.color, material.alpha);	\n"
	...
	{
		const float material_color[] = { 1.0f, 0.0f, 0.5f };
		const float material_alpha = 0.65f;

		GLuint ubo;
		glCreateBuffers(1, &ubo);

		// Get the indices of the uniform block members
		const char* uniform_names[] = {
			"SharedLayoutMaterialUniformBlock.color",
			"SharedLayoutMaterialUniformBlock.alpha"
		};
		const int num_uniform_members = sizeof(uniform_names)/sizeof(uniform_names[0]);  // It should be 2
		GLuint uniform_indices[num_uniform_members];
		glGetUniformIndices(program, num_uniform_members, uniform_names, uniform_indices);
		
		// Get information (uniform offset, array stride, matrix, etc.) from uniform block members
		GLint uniform_offsets[num_uniform_members];
		glGetActiveUniformsiv(program, num_uniform_members, uniform_indices, GL_UNIFORM_OFFSET, uniform_offsets);

		// Save uniform block members data in desired locations
		glNamedBufferStorage(ubo, uniform_offsets[num_uniform_members - 1] + sizeof(material_alpha), material_color, GL_DYNAMIC_STORAGE_BIT);
		glNamedBufferSubData(ubo, uniform_offsets[num_uniform_members - 1], sizeof(material_alpha), &material_alpha);

		GLuint uniform_index = glGetUniformBlockIndex(program, "SharedLayoutMaterialUniformBlock");
