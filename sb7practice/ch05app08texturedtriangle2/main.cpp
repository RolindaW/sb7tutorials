...
public:
	...
	void render(double currentTime)
	{
		...
		glDrawArrays(GL_TRIANGLES, 0, 3);  // Render triangle: count=3 - Render square: count=6
	}
...
private:
	void InitializeProgram()
	{
		...
		const GLchar* fragmentShaderSource[] = {
			...
			"void main(void)													\n"
			"{																	\n"
			"	//color = texture(s, vec2(3*uv.x, 2*uv.y)); Non-uniform scale	\n"
			"	//color = texture(s, 3*uv); Unifor scale 						\n"
			"	//color = texture(s, uv).rrra; Swizzle							\n"
			"	color = texture(s, uv);											\n"
			"}																	\n"
		};
